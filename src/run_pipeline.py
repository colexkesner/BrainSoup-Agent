...
from src.approval import VALID_HITL_MODES
from src.utils import PipelineConfig, append_jsonl, load_yaml, now_iso


def _ensure_dirs(cfg: dict) -> None:
    paths = [
        cfg["output_dirs"]["powerbi"],
        cfg["output_dirs"]["reports"],
        cfg["output_dirs"]["logs"],
        cfg["cache_dir"],
        cfg.get("approved_data_dir", "data/raw/approved"),
    ]
    for p in paths:
...
def _neighbors_comparison(df_5yr, neighbors: list[str]):
    if not neighbors:
        import pandas as pd
...
def _hitl_behavior(hitl_mode: str) -> tuple[bool, bool]:
    ...
    if hitl_mode == "auto_reject":
        return True, False
    if hitl_mode in {"noninteractive_ui", "noninteractive_prompt"}:
        return False, False
...
def run_pipeline(config_path: str, hitl_mode_override: str | None = None) -> None:
    ...
    if cfg.get("enable_hosted_container_tools"):
        append_jsonl(
            ...
            {
                "ts": now_iso(),
                "event": "hosted_container_tools_enabled_warning",
                "message": "Sessions may expire; log IDs externally.",
            },
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hitl-mode",
        choices=["interactive", "auto_reject", "noninteractive_ui", "noninteractive_prompt"],
        default=None,
    )
