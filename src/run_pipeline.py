...
def run_pipeline(config_path: str, hitl_mode_override: str | None = None) -> None:
    ...
    if len(pdf_text) < int(cfg.get("pdf_text_min_chars", 800)):
        append_jsonl(
            run_log,
            {
                ...
                "message": "Extracted PDF text is too short. Please provide text or approve OCR module use.",
            },
        )
    ...
    provenance = pd.concat([base_prov, approvals_prov, ingest_provenance], ignore_index=True, sort=False)
