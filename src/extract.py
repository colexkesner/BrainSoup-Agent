...
def load_excel_dataset(path: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    missing_cols = [c for c in COUNTY_COLUMNS if c not in county.columns]
    if missing_cols:
        raise ValueError(f"County sheet missing expected columns: {missing_cols}")
    ...
    if extra_years:
        warnings.append(f"Extra years detected: {extra_years}")
...
def extract_checkpoints(page_chunks: list[dict[str, str | int]], context_window: int = 1) -> pd.DataFrame:
    ...
    for chunk in page_chunks:
        ...
        for i, line in enumerate(lines):
            ...
            checkpoint_name = line.split(":")[0][:120] if ":" in line else line[:120]
