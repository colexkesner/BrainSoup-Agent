...
DATE_PATTERN = re.compile(r"\b(?:\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{1,2},?\s+\d{4})\b", re.IGNORECASE)
...
def extract_checkpoints(text: str) -> pd.DataFrame:
    rows: list[dict[str, str]] = []
    for line in [ln.strip() for ln in text.splitlines() if ln.strip()]:
        dates = DATE_PATTERN.findall(line)
        if dates:
            rows.append({"raw_line": line, "dates_found": " | ".join(dates)})
