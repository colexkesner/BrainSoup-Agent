...
REQUIRED_YEARS = {2018, 2019, 2021, 2022, 2023}
...
def load_excel_dataset(path: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    missing_required = sorted(REQUIRED_YEARS - years)
    if missing_required:
        raise ValueError(
            f"Missing required analysis years: {missing_required}. "
            f"Required subset is {sorted(REQUIRED_YEARS)}; available years={sorted(years)}"
        )

    warnings: list[str] = []
    extra_years = sorted(years - REQUIRED_YEARS)
    if extra_years:
