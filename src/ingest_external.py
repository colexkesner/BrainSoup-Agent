...
def ingest_approved_datasets(
    ...
    for ds in approvals.get("approved_datasets", []):
        ...
        base_keys = enriched[keys].drop_duplicates().copy()

        if "fips" in keys:
            dedup["fips"] = dedup["fips"].astype(str).str.zfill(5)
            base_keys["fips"] = base_keys["fips"].astype(str).str.zfill(5)
        if "year" in keys:
            dedup["year"] = dedup["year"].astype(int)
        ...
        if "fips" in keys:
            ...
            renamed["fips"] = renamed["fips"].astype(str).str.zfill(5)
        if "year" in keys:
