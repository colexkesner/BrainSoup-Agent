...
def pca_and_cluster(snapshot_2023: pd.DataFrame, seed: int, pca_components: int = 3) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, float]]:
    ...
    loadings["explained_variance_ratio"] = list(pca.explained_variance_ratio_) + [np.nan] * (
        len(loadings) - len(pca.explained_variance_ratio_)
    )
    ...
    for k in range(2, 9):
        ...
        labels2 = ag.fit_predict(z)
        sil2 = silhouette_score(z, labels2)
        db2 = davies_bouldin_score(z, labels2)
    ...
    cluster_df["distance_to_cluster_center"] = np.sqrt(
        (cluster_df["PC1"] - centroids["PC1"]) ** 2 + (cluster_df["PC2"] - centroids["PC2"]) ** 2
    )
...
def find_anomalies(df: pd.DataFrame, seed: int) -> pd.DataFrame:
    ...
    tmp["reason"] = np.where(
        tmp["delta_ALICE_pct"].abs() >= tmp["delta_Poverty_pct"].abs(),
        "ALICE_pct shift dominates",
        "Poverty_pct shift dominates",
    )

    return tmp[tmp["is_anomaly"]][
        ["fips", "County", "State", "year", "anomaly_score", "reason", "delta_ALICE_pct", "delta_Poverty_pct", "Gap_pct"]
